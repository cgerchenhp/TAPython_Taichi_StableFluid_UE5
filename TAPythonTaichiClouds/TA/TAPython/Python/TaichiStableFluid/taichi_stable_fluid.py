...
class MouseDataGen:
    def __init__(self):
        ...
        self.brush_color = np.array([0, 0.5, 1]) # bgr

    def __call__(self, cursor_pos, bLmb):
        # [0:2]: normalized delta direction
        # [2:4]: current mouse xy
        # [4:7]: color
        mouse_data = np.zeros(8, dtype=np.float32)
        if bLmb:
            mxy = np.array(cursor_pos, dtype=np.float32) * res
...
class StableFluid(metaclass=Singleton):
    def __init__(self, jsonPath:str):
        self.jsonPath = jsonPath
        self.data = unreal.PythonBPLib.get_chameleon_data(self.jsonPath)
        self.ui_image = "taichi_fluid_image"
        self.paused = False
        self.md_gen = MouseDataGen()
        self.cursor_pos = None
        self.bLMB = False
        self.tick_count = 0
        self.visualize_d = True
        self.visualize_v = False
        self.visualize_c = False
        self.bSRGB = False

        self.width = self.height = res
        self.image_im = np.ones((self.height, self.width, 4), dtype=np.uint8) * 255
        self.ui_output = "Output"
        self.rt = unreal.load_asset("/Game/Materials/RT_Fluid")
    ...
    def on_tick(self):
        if not self.paused:
            mouse_data = self.md_gen(self.cursor_pos, self.bLMB)
            step(mouse_data)
            if self.visualize_c:
                vorticity(velocities_pair.cur)
                u8_im = np.asarray(np.clip((velocity_curls.to_numpy() * 0.03) * 255, 0, 255), dtype=np.uint8)
            elif self.visualize_d:
                u8_im = np.asarray(dyes_pair.cur.to_numpy() * 255, dtype=np.uint8)
            elif self.visualize_v:
                u8_im = np.asarray(np.clip(velocities_pair.cur.to_numpy() * 0.01 * 255 + 128, 0, 255), dtype=np.uint8)

            im_sum = u8_im.sum()
            self.data.set_text(self.ui_output, f"im_sum: {im_sum}  {im_sum.shape}")
            if len(u8_im.shape) == 3:
                self.image_im[:, :, 0:u8_im.shape[2]] = u8_im[:, :, ]
            else:
                self.image_im[:, :, 0]= self.image_im[:, :, 1]=self.image_im[:, :, 2] = u8_im[:,:]

            image_bytes = self.image_im.data.tobytes()
            self.data.set_image_data(self.ui_image, image_bytes, width=self.width, height=self.height, bgr=False) # use blue color for good looking
            if self.rt:
                unreal.PythonTextureLib.set_render_target_data(self.rt, image_bytes, self.width, self.height, 4, self.bSRGB, bgr=True)
    ...
    def change_mode(self, mode_str : str):
        self.visualize_v = False
        self.visualize_c = False
        self.visualize_d = False
        if mode_str == 'v':
            self.visualize_v = True
        elif mode_str == 'd':
            self.visualize_d = True
        elif mode_str == 'c':
            self.visualize_c = True
        print(f"Change to Mode: {mode_str}")


    def on_mouse_move(self, uv, mouse_flags):
        self.cursor_pos = (uv[1], uv[0]) # axis is different, swap input axis rather than swap im's axis
        self.bLMB = mouse_flags % 2 == 1
